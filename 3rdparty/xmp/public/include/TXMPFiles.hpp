...
public:
    ...
    // ---------------------------------------------------------------------------------------------
    /// @brief \c ComputeChecksum() retrieves checksum of opened file's media data.
    ///
    /// @param checksum [out] A buffer in which to return string checksum value.
    ///
    /// @param XMPPacketSize [out] A buffer in which to return the XMP packet size
    /// or -1 if there is no XMP packet in the file
    ///
    /// @param XMPPacketOffset [out] A buffer in which to return the XMP packet offset
    /// or -1 if there is no XMP packet in the file

    bool ComputeChecksum (tStringObj * checksum,
                          XMP_Int64  * XMPPacketSize = 0,
                          XMP_Int64  * XMPPacketOffset = 0 );
