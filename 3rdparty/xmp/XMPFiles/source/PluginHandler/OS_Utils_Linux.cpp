...
OS_ModuleRef LoadModule( const std::string & inModulePath, bool inOnlyResourceAccess)
{
	...
	if( inOnlyResourceAccess )
	{
		...
		{	// success !
                        result = (OS_ModuleRef)fileHandle;
	...
	{
#if defined (XMP_MacBuild) || defined (XMP_iOSBuild)
                result = (OS_ModuleRef)dlopen(reinterpret_cast<const char*>(inModulePath.c_str()), RTLD_LAZY/*RTLD_NOW*/);
#else
		result = dlopen(reinterpret_cast<const char*>(inModulePath.c_str()), RTLD_LAZY/*RTLD_NOW*/);
#endif /* XMP_MacBuild || XMP_iOSBuild */
		if( !result )
		{
