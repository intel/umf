...
bool
XMPFiles::ComputeChecksum ( XMP_Uns8  * digest,
                            XMP_Int64 * XMPPacketSize/* = 0*/,
                            XMP_Int64 * XMPPacketOffset/* = 0*/) const
{
    XMP_FILES_START
    if ( this->handler == 0 ) return false;

    XMPFiles_IO * file = NULL;
    if(!this->ioRef)
        file = XMPFiles_IO::New_XMPFiles_IO(this->filePath.c_str(), true);
    else
        file = dynamic_cast<XMPFiles_IO*>(this->handler->parent->ioRef);

    if(!file) return false;

    XMP_Int64 packetSize = this->handler->packetInfo.length;
    if(XMPPacketSize) *XMPPacketSize = packetSize;

    XMP_Int64 packetOffset = this->handler->packetInfo.offset;
    if(XMPPacketOffset) *XMPPacketOffset = packetOffset;

    Host_IO::FileRef hostFile = file->GetFileRef();
    XMP_Int64 hostFileOffset = Host_IO::Offset(hostFile);
    Host_IO::Rewind(hostFile);

    MD5_CTX context;
    MD5Init(&context);

    int bufferSize = 1024 * 1024;
    std::unique_ptr<XMP_Uns8[]> buffer(new XMP_Uns8[bufferSize]);

    XMP_Int64 fileSize = file->Length();
    XMP_Int64 mediaSize = packetSize > 0 ? packetOffset - 24 : fileSize;
    XMP_Int64 currentOffset = 0;

    Host_IO::Seek(hostFile, 0, kXMP_SeekFromStart);
    while(currentOffset < mediaSize)
    {
        XMP_Int64 nReadBytes = mediaSize - currentOffset >= bufferSize ? bufferSize : mediaSize - currentOffset;
        currentOffset += Host_IO::Read(hostFile, buffer.get(), nReadBytes);
        MD5Update(&context, buffer.get(), nReadBytes);
    }

    if (packetSize  > 0 && packetOffset + packetSize < fileSize)
    {
        XMP_Uns32 nReadBytes = 0;
        Host_IO::Seek(hostFile, 24 + packetSize, kXMP_SeekFromCurrent);
        while( (nReadBytes = Host_IO::Read(hostFile, buffer.get(), bufferSize) ) > 0)
        {
            MD5Update(&context, buffer.get(), nReadBytes);
        }
    }
    ...
    Host_IO::Seek(hostFile, hostFileOffset, kXMP_SeekFromStart);
    if(this->ioRef == nullptr)
        delete file;

    XMP_FILES_END1 ( kXMPErrSev_FileFatal )
    return true;

}   // XMPFiles::ComputeChechsum
